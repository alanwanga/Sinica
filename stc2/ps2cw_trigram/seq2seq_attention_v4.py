...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, n_layers, dropout_p, attn_method):
        ...
        self.inputs_combine = nn.Linear(self.hidden_size * 2, self.hidden_size)        
    ...
    def forward(self, input_, input_p, hidden, cell, encoder_hiddens):
        embedded = self.embedding(input_)[:, 0, :]
        embedded = self.dropout(embedded)

        embedded_p = self.embedding(input_p)[:, 0, :]
        embedded_p = self.dropout(embedded_p)
        #import pdb; pdb.set_trace()
        embedded_combine = torch.cat((embedded, embedded_p), 1)
        embedded_combine = self.inputs_combine(embedded_combine)
        
        embedded_combine  = self.relu(embedded_combine)

        attn_applied, attn_weights = self.attn(hidden.squeeze(0), encoder_hiddens)

        output = torch.cat((embedded_combine, attn_applied), 1)
...
def train(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, teacher_forcing_ratio=0.5):
    ...
    decoder_input_p = Variable(torch.LongTensor([[SOS_TOKEN]*n_sample])).view(-1, 1)
    decoder_input_p = decoder_input_p.cuda(GPUID) if USE_CUDA else decoder_input_p
    ...
    for di in range(target_length):
        decoder_output, decoder_hidden, decoder_cell, decoder_attention = decoder(
            decoder_input, decoder_input_p, decoder_hidden, decoder_cell, encoder_hiddens)

        loss += criterion(decoder_output, target_variable[:, di])

        decoder_input_p = decoder_input
...
DATA_PATH = 'data'
SAVE_PATH = 'models'
SAVE_PREFIX = 'ps2cw-trigram-100000'
BUCKETS = [30]
FILE_PATH = 'sw-100000.txt'
EMBEDDING_PATH = ''
EMBEDDING_DIM = 300
