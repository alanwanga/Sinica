...
min_word_freq = 3
# set UNK if word2count is under threshold 

num_layers = 2
hidden_size = 128#512
max_length = 30
batch_size = 64
train_num = batch_size * 1600
...
def prepareData(lang1, lang2, max_length, reverse=False):
    ...
    try:
        ...
        for word in freq_count:
            if freq_count[word] < min_word_freq:
...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, n_layers=num_layers):
...
class DecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, n_layers=num_layers):
...
trainIters(encoder, decoder, epochs, batch_size, print_every=10, test_every=100)
